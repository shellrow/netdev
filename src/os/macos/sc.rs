...
pub fn map_sc_interface_type(type_id: &str) -> InterfaceType {
    match type_id {
...
///
/// This is used to obtain a friendly name and a more accurate interface type.
pub(crate) fn get_sc_if_map() -> HashMap<String, SCInterface> {
...
fn load_networkinterfaces_plist_map(bytes: &[u8]) -> HashMap<String, SCInterface> {
...
///
/// This is used to obtain a friendly name and a more accurate interface type.
pub(crate) fn read_networkinterfaces_plist_map() -> std::io::Result<HashMap<String, SCInterface>> {
    let bytes = std::fs::read(SC_NWIF_PATH)?;
    Ok(load_networkinterfaces_plist_map(&bytes))
}
